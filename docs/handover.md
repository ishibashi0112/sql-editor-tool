...
### 14.3 確認手順：レポート（0.4.0〜0.5.0、2026-09-26）

0.5.0 の vsix を入れて、SQL Server の接続で確かめる。持ち帰るものは §14 と同じ（業務 SQL・業務データは伏せる）。
...
6. Oracle の接続でも 1〜5（推定はしないので、2 は「すべて文字列」になれば正しい）。
